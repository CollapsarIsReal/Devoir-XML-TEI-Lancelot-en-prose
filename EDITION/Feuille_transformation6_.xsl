...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="//unclear" mode="#all">
        <xsl:choose> <!-- gestion des mots que nous ne sommes pas parvenus à transcrire -->
                    ...
                    <xsl:for-each select="ancestor::TEI//body//persName[replace(@ref, '#','')=$idPerson]">
                        <xsl:choose>
                            <xsl:when test="ancestor::ab">
                                <div> <!-- il aurait été bien de faire un groupe-by par ab[@n] pour éviter les répétitions
                                    de feuillets, mais je n'y suis pas parvenu -->
                                    <xsl:value-of select="concat('Feuillet n°', ancestor::ab/@n[position()=last()]), ' - '"/> 
