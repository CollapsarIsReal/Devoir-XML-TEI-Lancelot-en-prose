...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="titre_manuscrit">
            <xsl:value-of
                select="concat(.//head/title, ' ', '(', .//head/origDate, ')', ' de ', .//name[@xml:id = 'Gauthier_Map'])"
            />
        </xsl:variable>
        ...
        <xsl:result-document href="{$pathAccueil}" method="html" indent="yes">
            <html>
                ...
                <body>
                    <h1>
                        <xsl:value-of select="$titre_manuscrit"/>
                    </h1>
                    <p><xsl:value-of
                            select="concat('Cette édition en ligne a été réalisée à partir du manuscrit ', $ms_bnf, ' conservé à la ', .//institution, '. ')"/>
                        <xsl:value-of
                            select="concat('Une feuille de transformation XSL a été rédigée à partir ', 'd une ', $responsable, ' ', $ms_bnf, ', réalisés par ', .//editionStmt/respStmt//forename, ' ', .//editionStmt/respStmt//surname, '. ')"
                        /> Les choix éditoriaux ont été les suivants. Le terme "Chapitre" a été
                        privilégié à celui de "rubrique" pour distinguer les parties du manuscrit
                        transcrites et encodées. Les transcriptions ont été segmentées en
                        paragraphes afin de faciliter la compréhension du texte.</p>
                    ...
                    <div type="sommaire">
                        ...
                        <ul>
                            <li>
                                <a href="{$pathnoticems}">
                                    <xsl:value-of select="concat('Notice du manucrit ', $ms_bnf)"/>
                                </a>
                            </li>
                            <li>
                                <a href="{$pathchap}">
                                    <xsl:call-template name="titre_chapitre"/>
                                </a>
                            </li>
                            <li>
                                <a href="{$pathAllo}">La transcription allographétique</a>
                            </li>
                            <li>
                                <a href="{$pathNorm}">La transcription normalisée</a>
                            </li>
                            <li>
                                <a href="{$pathIndexPers}">L'index des noms de personnages</a>
                            </li>
                            <li>
                                <a href="{$pathIndexLieux}">L'index de lieux</a>
                            </li>
                        </ul>
                    </div>
            ...
            <html>
                ...
                <body>
                    ...
                    <dl>
                        ...
                        <dd>
                            <xsl:value-of select=".//altIdentifier[1]/idno"/>
                        </dd>
                    </dl>

                    <h3>L'histoire de la conservation du manuscrit :</h3>
                    <p>
                        <xsl:value-of select=".//origin | .//provenance/p"/>
                    </p>
                    ...
                    <dl>
                        ...
                        <dd>
                            <xsl:value-of select=".//msItem/respStmt[1]/name[1]"/>
                        </dd>
                        <dt>
                            <b>Enlumineur :</b>
                        </dt>
                        <dd>
                            <xsl:value-of
                                select="concat(.//msItem/respStmt[2]/name, ' est un ', replace(.//msItem/respStmt[2]/note[1], 'Enlumineur', 'enlumineur'))"
                            />
                        </dd>
                    </dl>
                    ...
                    <dl>
                        ...
                        <dd>
                            <xsl:value-of
                                select="concat(.//decoDesc/decoNote[1]/@type, ', ', .//decoDesc/decoNote[2]/@type, ' ornées')"
                            />
                        </dd>
                        <dd>Les décorations sont davantages détaillées dans la présentation de
                            chaque chapitre/rubrique du manuscrit qui précède les
                            transcriptions.</dd>
                        <dt>
                            <b>Reliure :</b>
                        </dt>
                        <dd>
                            <xsl:value-of
                                select=".//decoNote[@type = 'reliure'] | .//decoNote[@type = 'armoiries']"
                            />
                        </dd>
                        <dt>
                    ...
                    <dl>
                        <dt>
                            <xsl:value-of
                                select="concat('(', .//msContents/msItem/locus/@from, ' à ', .//msContents/msItem/locus/@to, ') ', .//msContents/msItem/locus)"
                            />
                        </dt>
                        <dt>
                            <b>Incipit :</b>
        ...
        <xsl:result-document href="{$pathchap}" method="html" indent="yes">
            <html>
                <head>
                    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
                    <title>
                ...
                <body>
                    ...
                    <p>
                        <xsl:value-of select="concat('La rubrique : ', '''', .//rubric, '''')"/>
                    </p>

                    <p>
                        <xsl:value-of
                            select="concat('La transcriptions allographétique et celle normalisée du chapitre démarrent au ', replace(.//msContents/msItem/msItem/locus, 'f.', 'folio'))"
                        />
                    </p>

                    <p>Transcription des folios : <a href="{.//pb[@n='101r']/@facs}"><xsl:value-of
                                select=".//msContents/msItem/msItem/locus/@from"/></a>, <a
                            href="{.//pb[@n='101v']/@facs}"><xsl:value-of
                                select=".//msContents/msItem/msItem/locus/@to"/></a></p>
    ...
    <xsl:template match="decoDesc/decoNote/p">
        <xsl:copy>
            <xsl:choose>
                <xsl:when test=".//list"> Mots clefs : <xsl:for-each select=".//item">
                        <xsl:value-of select="."/>
                        <xsl:choose>
    ...
    <xsl:template name="titre_chapitre">
        <xsl:if test=".//text/body//div[@type = 'chapitre']">
            <xsl:value-of
                select="concat(upper-case(.//div/div/@type), ' ', .//div/div/@n, ' : ', 'La ', replace(replace(.//div/div/@subtype, '#', ''), '_', ' de '))"
            />
        </xsl:if>
