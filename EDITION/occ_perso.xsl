    ...
    <xsl:template name="indexPers">
        <xsl:for-each select="//listPerson/person/persName">
            <li>
                ...
                <div><xsl:text>Nombre d'occurences :</xsl:text><xsl:value-of select="$nb-occurrence"
                    /></div> PLOP<p> <xsl:for-each
                        select="ancestor::TEI//body//persName[replace(@ref, '#', '') = $idPerson]">
                        <xsl:choose>
                            <xsl:when test="ancestor::ab">
                                <div>Feuillet n° <xsl:value-of
                                        select="ancestor::ab/@n[position() = last()]"/> Colonne n°
                                        <xsl:value-of select="parent::cb/@n"/></div>
                                <xsl:choose>
